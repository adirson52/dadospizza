...
let ordersData = []; // Variável global para armazenar os pedidos
    ...
    try {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const order = {
                idCliente: cols[0],
                nomeCliente: cols[1],
                distancia: parseFloat(cols[2].replace(",", ".")),
                tipoEntrega: cols[3],
                tipoPagamento: cols[4],
                data: date,
                hora: time,
                pedido: cols[6].replace(/^"|"$/g, ''),
                valorTotal: parseFloat(cols[7].replace(",", ".")),
                quantidadeItens: parseInt(cols[8]),
                temPizza: cols[9] === '1' || cols[9].toLowerCase() === 'sim' ? 'Sim' : 'Não',
                temBebida: cols[10] === '1' || cols[10].toLowerCase() === 'sim' ? 'Sim' : 'Não'
            };
